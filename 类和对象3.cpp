...
// 需要进行运算符重载来进行自定义类型的计算
// 运算符重载是具有特殊函数名的函数, 函数原型: 返回值类型 Operate+(参数列表)
// 参数中必须至少有一个自定义类型的参数, 不能全是内置类型的
// 如果运算符重载函数为成员函数时,需要显示定义的参数个数比操作符要求的个数少一个, 因为会编译器自动添加一个this指针作为其函数内部的第一个参数, 从而造成参数过多 _a就相当于this->_a
// '.*' 、'::' 、'sizeof' 、'?:' 、'.' 注意以上5个运算符不能重载。

// 参数类型:
// 值: 传参发生拷贝, 且不能优化自赋值的逻辑
// 引用: 传参不发生拷贝, 可以优化自赋值的逻辑
// 返回值类型:
//	void: 不能进行连续赋值
// 引用/值: 可以进行连续赋值, 如果是引用, 效率更高
// return *this;
// 进行连续性赋值, 返回值绝对不能为void, 且连续赋值是从右向左 d1 = (d2 = d3)

// 创建新的对象 ---> 构造函数 Data d2 = d1;
// 赋值运算符 只是去改变已有对象的内容, 不会去创建新的对象 

// this是一各函数的形参, 谁调就指向谁 , 各自函数内部的局部变量, *this就是函数的本体

// 编辑器默认生成的赋值运算符也是一个浅拷贝

// 加法会改变操作数本身的内容-->返回引用 减法不会改变操作数本身的内容-->返回值即可
